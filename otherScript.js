...
saveButton.addEventListener("click",()=>{
    ...
    console.log(myArray);
    localStorage.setItem("other-task",JSON.stringify(myArray));
    alert("Task Saved");
})
...
function getTask(){
                          const pending = JSON.parse(localStorage.getItem("other-task"));
                          console.log(pending);
...
const saveNotes = () =>{
  ...
  console.log(myArray);
  localStorage.setItem("other-notes", JSON.stringify(myArray));
}


const getlocalData = () => {
  const lsnotes = JSON.parse(localStorage.getItem("other-notes"));
  console.log(lsnotes);
